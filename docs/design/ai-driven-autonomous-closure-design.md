...
**不引入新的强 Workflow 引擎**，完全利用现有架构实现 AI 自主闭环。

## 适用场景

这套系统可以支持多种任务类型，不限于代码开发：
...
| 原则 | 说明 |
|-----|------|
| **可接受参数** | 可以接收特定 Issue/PR 编号，而不是扫描所有 |
| **聚焦单个任务** | 每次只处理一个指定的 Issue/PR |
| **结果可追踪** | 执行结果可以被管家心跳获取 |

#### 4.2 心跳参数传递机制

通过心跳的 `mdContent` 中的变量替换，或通过需求的 `AcceptanceCriteria` 传递参数：
...
- [ ] 实现状态机初始化逻辑
- [ ] 将管家心跳加入内置场景
- [ ] 集成测试：验证管家心跳可以正常扫描和创建元需求

### Phase 3: 专用心跳改造（优先级：P1）

**目标**：让现有 8 个心跳可以被参数化调用
...
| **状态机过于复杂** | 中 | 中 | 保持状态机简洁；提供默认配置；允许用户自定义 |
| **改造工作量大** | 中 | 中 | 分阶段实施；保持向后兼容；先验证核心流程 |
| **现有心跳功能受影响** | 高 | 低 | 保持现有心跳独立运行能力；新功能作为增强而非替代 |
...
| **保持灵活性** | 没有硬编码的工作流，AI 可以灵活调整 |
| **渐进式演进** | 可以分阶段实施，风险可控 |
| **向后兼容** | 保留现有独立心跳功能 |
| **支持多场景** | 基于 GitHub 可以支持代码开发、专利撰写、剧本写作、调研报告等多种任务 |
...
3. **无需额外基础设施**：降低成本和复杂度
4. **可扩展性强**：通过不同的仓库、标签、工作流配置，可以支持各种场景

# Claude Code SDK 配置项文档

## 概述

本文档列出 Claude Code SDK 支持的所有配置项，用于指导 Agent 编辑界面中 CodingAgent 的配置功能开发。
...
- **类型**: `string`
- **说明**: 当主模型不可用时使用的备用模型
- **SDK 方法**: `WithFallbackModel(model string)`

### 2.3 SystemPrompt - 系统提示词
- **类型**: `string`
- **说明**: 设置系统级提示词，定义 Agent 的行为和角色
- **SDK 方法**: `WithSystemPrompt(prompt string)`

### 2.4 AppendSystemPrompt - 追加系统提示词
- **类型**: `string`
- **说明**: 在现有系统提示词后追加内容
- **SDK 方法**: `WithAppendSystemPrompt(prompt string)`

### 2.5 MaxThinkingTokens - 最大思考 Token 数
- **类型**: `int`
- **说明**: 允许模型用于思考的最大 Token 数量
- **默认值**: `8000`
- **SDK 方法**: `WithMaxThinkingTokens(tokens int)`
...
- **说明**: 控制工具调用的权限处理方式
- **SDK 方法**: `WithPermissionMode(mode PermissionMode)`
- **可选值**:
...
- **说明**: 动态控制工具使用权限的回调函数
- **SDK 方法**: `WithCanUseTool(callback CanUseToolCallback)`
...
- **SDK 方法**: `WithResume(sessionID string)`
- **用途**: 实现多轮对话支持

### 4.2 MaxTurns - 最大对话轮次
- **类型**: `int`
- **说明**: 限制会话中的最大交互次数
- **SDK 方法**: `WithMaxTurns(turns int)`
- **示例**: `WithMaxTurns(10)`

### 4.3 ContinueConversation - 继续会话
- **类型**: `bool`
- **说明**: 是否启用会话继续功能
- **SDK 方法**: `WithContinueConversation(continueConversation bool)`

### 4.4 ForkSession - Fork 会话
- **类型**: `bool`
- **说明**: 恢复会话时是否 fork 到新 Session ID
- **SDK 方法**: `WithForkSession(fork bool)`
...
- **SDK 方法**: `WithCwd(cwd string)`
- **示例**: `WithCwd("/path/to/project")`

### 5.2 AddDirs - 添加目录
- **类型**: `[]string`
- **说明**: 将指定目录添加到上下文
- **SDK 方法**: `WithAddDirs(dirs ...string)`

### 5.3 FileCheckpointing - 文件检查点
- **类型**: `bool`
- **说明**: 启用文件变更跟踪，支持回滚到之前状态
- **SDK 方法**: `WithFileCheckpointing()` 或 `WithEnableFileCheckpointing(enable bool)`
- **用途**: 撤销文件修改、文件历史管理
...
- **SDK 方法**: `WithMcpServers(servers map[string]McpServerConfig)`
- **服务器类型**:
...
- **说明**: 在进程中运行的 MCP 服务器
- **SDK 方法**: `WithSdkMcpServer(name string, server *McpSdkServerConfig)`
...
- **类型**: `[]SdkPluginConfig`
- **说明**: 配置要加载的插件
- **SDK 方法**: `WithPlugins(plugins []SdkPluginConfig)`

### 8.2 LocalPlugin - 本地插件
- **类型**: `string` (路径)
- **说明**: 从本地路径加载插件
- **SDK 方法**: `WithLocalPlugin(path string)`
...
- **SDK 方法**: `WithJSONSchema(schema map[string]any)`
- **用途**: 提取结构化数据、类型安全响应

### 9.2 OutputFormat - 输出格式
- **类型**: `OutputFormat`
- **说明**: 设置响应输出格式
- **SDK 方法**: `WithOutputFormat(format *OutputFormat)`
...
- **SDK 方法**: `WithPreToolUseHook(matcher string, callback HookCallback)`

### 10.2 PostToolUse - 工具执行后钩子
- **触发时机**: 工具执行后
- **用途**: 添加上下文、修改输出、审计追踪
- **SDK 方法**: `WithPostToolUseHook(matcher string, callback HookCallback)`

### 10.3 UserPromptSubmit - 用户提交提示时
- **触发时机**: 用户提交提示词时
- **用途**: 提示词验证、上下文注入
- **SDK 方法**: `WithHook(HookEventUserPromptSubmit, matcher, callback)`

### 10.4 Stop - 会话停止时
- **触发时机**: 会话结束前
- **用途**: 资源清理、最终状态保存
- **SDK 方法**: `WithHook(HookEventStop, matcher, callback)`

### 10.5 SubagentStop - 子代理停止时
- **触发时机**: 子代理会话结束时
- **SDK 方法**: `WithHook(HookEventSubagentStop, matcher, callback)`

### 10.6 PreCompact - 上下文压缩前
- **触发时机**: 上下文压缩/精简前
- **用途**: 选择性保留信息
- **SDK 方法**: `WithHook(HookEventPreCompact, matcher, callback)`
...
- **类型**: `[]SdkBeta`
- **说明**: 启用实验性功能
- **SDK 方法**: `WithBetas(betas ...SdkBeta)`
- **可用值**:
...
- **类型**: `float64`
- **说明**: API 使用的最大预算 (USD)
- **SDK 方法**: `WithMaxBudgetUSD(budget float64)`

### 12.2 IncludePartialMessages - 部分消息流
- **类型**: `bool`
- **说明**: 启用流式部分消息更新
- **SDK 方法**: `WithIncludePartialMessages(include bool)`

### 12.3 DebugWriter - 调试输出
- **类型**: `io.Writer`
- **说明**: CLI 调试输出目标
- **SDK 方法**: `WithDebugWriter(w io.Writer)`
- **常用值**: `os.Stderr`, `io.Discard`

### 12.4 StderrCallback - 标准错误回调
- **类型**: `func(string)`
- **说明**: 接收 CLI stderr 输出
- **SDK 方法**: `WithStderrCallback(callback func(string))`

### 12.5 CLIPath - CLI 路径
- **类型**: `string`
- **说明**: 自定义 Claude Code CLI 路径
- **SDK 方法**: `WithCLIPath(path string)`

### 12.6 Env - 环境变量
- **类型**: `map[string]string`
- **说明**: 为子进程设置环境变量
- **SDK 方法**: `WithEnv(env map[string]string)`
- **示例**: `WithEnv(map[string]string{"ANTHROPIC_API_KEY": "sk-xxx"})`

### 12.7 ExtraArgs - 额外 CLI 参数
- **类型**: `map[string]*string`
- **说明**: 传递额外 CLI 参数
- **SDK 方法**: `WithExtraArgs(args map[string]*string)`

### 12.8 Settings - 设置文件
- **类型**: `string`
- **说明**: 设置文件路径或 JSON 字符串
- **SDK 方法**: `WithSettings(settings string)`

### 12.9 SettingSources - 设置来源
- **类型**: `[]SettingSource`
- **说明**: 指定加载哪些设置源
- **SDK 方法**: `WithSettingSources(sources ...SettingSource)`

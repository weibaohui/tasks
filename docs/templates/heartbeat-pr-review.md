...
```bash
taskmanager requirement create \
  --project-id YAcIqbrV6KZF2gUnNpU0x \
  --title "[修复] 修复用户密码验证逻辑" \
  --description "## 背景
来源：PR #123 https://github.com/owner/repo/pull/123
评论：reviewer指出密码验证逻辑存在边界问题
...
函数：ValidatePassword
问题：第45行未对空密码做校验
修复：在函数开头添加 if password == nil || password == \"\" { return ErrPasswordEmpty }

## 修复步骤
1. 在ValidatePassword函数开头添加空密码校验
2. 添加单元测试覆盖空密码场景

## 验收标准
1. 空密码返回明确错误 ErrPasswordEmpty
2. 单元测试通过
3. 现有功能不受影响" \
  --acceptance "1. 空密码返回ErrPasswordEmpty
2. 单元测试通过"
```

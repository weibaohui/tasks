...
func (s *HeartbeatScenarioService) CreateScenario(ctx context.Context, code, name, description string, items []domain.HeartbeatScenarioItem) (*domain.HeartbeatScenario, error) {
...
// EnsureBuiltInScenarios 确保内置场景存在
func (s *HeartbeatScenarioService) EnsureBuiltInScenarios(ctx context.Context) error {
	githubDevScenario := BuildGitHubDevWorkflowScenario(s.idGenerator.Generate())

	existing, err := s.scenarioRepo.FindByCode(ctx, githubDevScenario.Code())
	if err != nil {
	...
	if err := s.scenarioRepo.Save(ctx, githubDevScenario); err != nil {
	...
	items := []domain.HeartbeatScenarioItem{
		{
			...
			MDContent: "你是项目的自动化协作助手。当前任务是：分析项目中的 open issues。\n\n" +
				...
				"2. 对每个未被分析过的 issue（检查评论中是否已有分析结论），clone 仓库到临时目录，结合代码库分析 issue 描述。\n" +
				"3. 将分析结论（问题根因、可能影响的文件、建议修复方向）以评论形式发布到该 issue 下。\n" +
				"4. 如果没有 open issues，直接返回\"当前无待分析 issue\"。\n\n" +
				"## 约束\n" +
				"- 使用 gh CLI 操作 GitHub。\n" +
				"- 不修改任何源代码。\n" +
				"- 每次最多分析 3 个 issue。",
		},
		{
			...
			MDContent: "你是项目的自动化协作助手。当前任务是：为已评审通过的 issue 编写代码并创建 PR。\n\n" +
				...
				"2. 对最旧的一个 issue：clone 仓库、创建 feature 分支、根据 issue 描述实现代码修改。\n" +
				"3. 如有测试命令，运行基础测试确保通过。\n" +
				"4. push 分支后使用 gh pr create 创建 PR，并在描述中通过 Closes #issue_number 关联 issue。\n" +
				"5. 如果没有带 lgtm 标签的 open issue，直接返回\"当前无待编写代码的 issue\"。\n\n" +
				"## 约束\n" +
				"- 使用 gh CLI 操作 GitHub。\n" +
				"- 每次心跳只处理一个 issue。\n" +
				"- 如遇到编译/测试失败，在 PR 或 issue 下评论说明。",
		},
		{
			Name:            "PR 需求评审",
			IntervalMinutes: 30,
			...
			MDContent: "你是项目的自动化协作助手。当前任务是：检查 open PR 是否已完成需求评审。\n\n" +
				...
				"2. 检查每个 PR 的评论中是否包含\"需求评审通过\"字样。\n" +
				"3. 若不存在，通过 PR 描述或 API 查找关联的 issue，阅读 issue 内容后在 PR 下评论：\n" +
				"   - 原始需求摘要\n" +
				"   - 需求评审结论（通过/需补充信息）\n" +
				"4. 如果所有 PR 都已通过需求评审，直接返回\"所有 PR 已完成需求评审\"。\n\n" +
				"## 约束\n" +
				"- 使用 gh CLI 操作 GitHub。\n" +
				"- 不修改任何源代码。",
		},
		{
			...
			MDContent: "你是项目的自动化协作助手。当前任务是：对 open PR 进行代码质量和安全评审。\n\n" +
				...
				"## 执行步骤\n" +
				"1. 使用 gh pr list --repo owner/repo --state open 获取 open PRs。\n" +
				"2. 对每个未被评审过的 PR，使用 gh pr view 和 gh pr diff 查看变更。\n" +
				"3. 从代码质量、潜在 bug、安全漏洞、性能问题等角度进行评审，并将评审意见以评论形式发布到 PR 下。\n" +
				"4. 如果没有待评审 PR，直接返回\"当前无待评审 PR\"。\n\n" +
				"## 约束\n" +
				"- 使用 gh CLI 操作 GitHub。\n" +
				"- 不修改任何源代码。\n" +
				"- 每次最多评审 2 个 PR。",
		},
		{
			...
			MDContent: "你是项目的自动化协作助手。当前任务是：修复 PR 中的修改建议。\n\n" +
				...
				"2. 阅读 PR 评论中的修改建议，判断哪些是可执行的合理建议。\n" +
				"3. checkout PR 分支、按建议修改代码、commit、push 更新。\n" +
				"4. 在 PR 下评论说明已修复的内容。\n" +
				"5. 如果没有待修复建议，直接返回\"当前无待修复的 PR\"。\n\n" +
				"## 约束\n" +
				"- 使用 gh CLI 操作 GitHub。\n" +
				"- 每次只处理一个 PR 的一批建议。\n" +
				"- 修复后尽量运行本地测试。",
		},
		{
			...
			MDContent: "你是项目的自动化协作助手。当前任务是：检查 PR 是否达到可合并状态。\n\n" +
				...
				"## 执行步骤\n" +
				"1. 使用 gh pr list --repo owner/repo --state open 获取 open PRs。\n" +
				"2. 对每个 PR 检查：CI 是否通过（gh pr checks）、是否有至少一条 /lgtm 或类似批准评论、是否有未解决的修改建议。\n" +
				"3. 若满足合并条件且尚未被标记为可合并，在 PR 下评论 /lgtm。\n" +
		...
		},
		{
			...
			MDContent: "你是项目的自动化协作助手。当前任务是：根据 PR 代码变更补充或更新文档。\n\n" +
				...
				"默认分支：${project.default_branch}\n\n" +
				"## 执行步骤\n" +
				"1. 使用 gh pr list --repo owner/repo --state open 获取近期有代码变更的 PR。\n" +
				"2. 查看 PR diff，判断是否需要更新 README、API 文档、变更日志等。\n" +
				"3. 若需要，在 PR 分支上补充文档并 push，在 PR 下评论说明更新的内容。\n" +
				"4. 如果无需补充文档，直接返回\"当前 PR 无需补充文档\"。\n\n" +
				"## 约束\n" +
				"- 使用 gh CLI 操作 GitHub。\n" +
				"- 每次最多处理 2 个 PR。\n" +
				"- 文档变更应与代码变更对应。",
		},
		{
			...
			MDContent: "你是项目的自动化协作助手。当前任务是：根据 PR 代码变更补充测试用例。\n\n" +
				...
				"## 执行步骤\n" +
				"1. 使用 gh pr list --repo owner/repo --state open 获取 open PRs。\n" +
				"2. 查看 PR diff，识别新增/修改的功能点，判断是否需要补充单元测试、集成测试。\n" +
				"3. 若需要，在 PR 分支上编写并补充相关测试，运行测试确保通过，push 更新。\n" +
				"4. 在 PR 下评论说明补充的测试内容。\n" +
				"5. 如果所有 PR 测试都已充足，直接返回\"当前 PR 无需补充测试\"。\n\n" +
				"## 约束\n" +
				"- 使用 gh CLI 操作 GitHub。\n" +
				"- 每次最多处理 1 个 PR。\n" +
				"- 补充的测试必须通过本地运行。",

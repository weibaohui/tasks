...
// PreClaudeCodeCall 记录 Claude Code 调用前的用户输入
func (h *ConversationRecordHook) PreClaudeCodeCall(ctx *domain.HookContext, callCtx *domain.ClaudeCodeCallContext) (*domain.ClaudeCodeCallContext, error) {
	if callCtx == nil {
		return nil, nil
	}
	...
	// 提取范围信息
	scope := h.extractClaudeCodeScope(ctx, callCtx)

	// 存储 scope 到 context
	ctx.WithValue(ScopeKey, scope)
	ctx.WithValue(spanKey, spanID)
	ctx.WithValue(promptKey, callCtx.UserInput)

	// 记录用户输入
	userInput := callCtx.UserInput
	if userInput == "" {
		userInput = callCtx.Prompt
	}
	record, err := h.createRecord(callCtx.TraceID, spanID, parentSpanID, "claude_code_call", "user", userInput)
	if err != nil {
		h.logger.Error("Failed to create conversation record for Claude Code user input", zap.Error(err))
		return callCtx, nil
	}

	record.SetScope(scope.SessionKey, scope.UserCode, scope.AgentCode, scope.ChannelCode, scope.ChannelType)
	if err := h.repo.Save(ctx.Context, record); err != nil {
		h.logger.Error("Failed to save conversation record for Claude Code user input", zap.Error(err))
	}

	h.logger.Debug("ConversationRecord: saved Claude Code user prompt",
		zap.String("trace_id", callCtx.TraceID),
		zap.String("span_id", spanID),
		zap.String("parent_span_id", parentSpanID),
		zap.Int("prompt_len", len(callCtx.Prompt)))
...
// PostClaudeCodeCall 记录 Claude Code 响应
func (h *ConversationRecordHook) PostClaudeCodeCall(ctx *domain.HookContext, callCtx *domain.ClaudeCodeCallContext, resp *domain.ClaudeCodeResponse) (*domain.ClaudeCodeResponse, error) {
	if resp == nil {
		return resp, nil
	}

	// 获取用户输入的 span_id 作为 parent
	parentSpanID, _ := ctx.Get(spanKey).(string)
	if parentSpanID == "" {
		parentSpanID = trace.GetSpanID(ctx.Context)
	}
	...
	// 记录助手响应
	record, err := h.createRecord(traceID, spanID, parentSpanID, "claude_code_response", "assistant", resp.Content)
	if err != nil {
		h.logger.Error("Failed to create conversation record for Claude Code response", zap.Error(err))
		return resp, nil
	}
	...
	if err := h.repo.Save(ctx.Context, record); err != nil {
		h.logger.Error("Failed to save conversation record for Claude Code response", zap.Error(err))
	}

	h.logger.Debug("ConversationRecord: saved Claude Code response",
		zap.String("trace_id", traceID),
		zap.String("span_id", spanID),
		zap.String("role", "assistant"),
		zap.Int("content_len", len(resp.Content)))

	return resp, nil
}

// OnClaudeCodeError 记录 Claude Code 错误
func (h *ConversationRecordHook) OnClaudeCodeError(ctx *domain.HookContext, callCtx *domain.ClaudeCodeCallContext, resp *domain.ClaudeCodeResponse) {
	if callCtx == nil {
		return
	}
	...
	record, err := h.createRecord(traceID, spanID, parentSpanID, "claude_code_error", "assistant", content)
	if err != nil {
		h.logger.Error("Failed to create conversation record for Claude Code error", zap.Error(err))
		return
	}
	...
	if err := h.repo.Save(ctx.Context, record); err != nil {
		h.logger.Error("Failed to save conversation record for Claude Code error", zap.Error(err))
	}

	h.logger.Debug("ConversationRecord: saved Claude Code error",
		zap.String("trace_id", traceID),
		zap.String("span_id", spanID))
}

// extractClaudeCodeScope 从 ClaudeCodeCallContext 提取范围信息
func (h *ConversationRecordHook) extractClaudeCodeScope(ctx *domain.HookContext, callCtx *domain.ClaudeCodeCallContext) ScopeInfo {
	scope := ScopeInfo{}

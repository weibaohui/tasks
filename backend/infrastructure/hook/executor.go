...
// ExecutePreClaudeCodeCall 执行 PreClaudeCodeCall 钩子
func (e *Executor) ExecutePreClaudeCodeCall(ctx *domain.HookContext, callCtx *domain.ClaudeCodeCallContext) (*domain.ClaudeCodeCallContext, error) {
	hooks := e.getAllEnabledHooks()
	hooks = e.sortByPriority(hooks)

	modifiedCtx := callCtx
	for _, hook := range hooks {
		ccHook, ok := hook.(domain.ClaudeCodeHook)
		if !ok {
			continue
		}

		e.logger.Debug("executing PreClaudeCodeCall",
			zap.String("hook", hook.Name()),
			zap.Int("priority", hook.Priority()))

		result, err := ccHook.PreClaudeCodeCall(ctx, modifiedCtx)
		if err != nil {
			e.logger.Error("PreClaudeCodeCall failed",
				zap.String("hook", hook.Name()),
				zap.Error(err))

			ctx.AddError(err, hook.Name(), "pre_claude_code_call")
...
// ExecutePostClaudeCodeCall 执行 PostClaudeCodeCall 钩子
func (e *Executor) ExecutePostClaudeCodeCall(ctx *domain.HookContext, callCtx *domain.ClaudeCodeCallContext, resp *domain.ClaudeCodeResponse) (*domain.ClaudeCodeResponse, error) {
	hooks := e.getAllEnabledHooks()
	hooks = e.sortByPriority(hooks)
	...
	for _, hook := range hooks {
		...
		e.logger.Debug("executing PostClaudeCodeCall",
			zap.String("hook", hook.Name()),
			zap.Int("priority", hook.Priority()))

		result, err := ccHook.PostClaudeCodeCall(ctx, callCtx, modifiedResp)
		if err != nil {
			e.logger.Error("PostClaudeCodeCall failed",
				zap.String("hook", hook.Name()),
				zap.Error(err))

			ctx.AddError(err, hook.Name(), "post_claude_code_call")
...
// ExecuteOnClaudeCodeError 执行 OnClaudeCodeError 钩子
func (e *Executor) ExecuteOnClaudeCodeError(ctx *domain.HookContext, callCtx *domain.ClaudeCodeCallContext, resp *domain.ClaudeCodeResponse) {
	hooks := e.getAllEnabledHooks()
	hooks = e.sortByPriority(hooks)
	...
	for _, hook := range hooks {
		...
		e.logger.Debug("executing OnClaudeCodeError",
			zap.String("hook", hook.Name()),
			zap.Int("priority", hook.Priority()))

		ccHook.OnClaudeCodeError(ctx, callCtx, resp)
		ctx.AddHook(hook.Name())
	}
}

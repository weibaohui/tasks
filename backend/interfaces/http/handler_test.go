...
func TestGetTask(t *testing.T) {
	...
	createBody := `{"name": "测试任务", "task_requirement": "测试目标", "acceptance_criteria": "测试验收标准", "type": "agent", "timeout": 60}`
...
func TestListTasksByTrace(t *testing.T) {
	...
	createBody := `{"name": "测试任务1", "task_requirement": "测试目标1", "acceptance_criteria": "测试验收标准1", "type": "agent", "timeout": 60}`
	createReq := httptest.NewRequest(http.MethodPost, "/api/v1/tasks", bytes.NewBufferString(createBody))
	createReq.Header.Set("Content-Type", "application/json")
	createW := httptest.NewRecorder()
	mux.ServeHTTP(createW, createReq)
...
func TestGetTaskTree(t *testing.T) {
	...
	createBody := `{"name": "父任务", "task_requirement": "父任务目标", "acceptance_criteria": "父任务验收标准", "type": "agent", "timeout": 60}`
	createReq := httptest.NewRequest(http.MethodPost, "/api/v1/tasks", bytes.NewBufferString(createBody))
	createReq.Header.Set("Content-Type", "application/json")
	createW := httptest.NewRecorder()
	mux.ServeHTTP(createW, createReq)

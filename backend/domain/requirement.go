...
// ProgressData 返回进度数据（领域值对象）
func (r *Requirement) ProgressData() *ProgressData {
	return r.progressData
}

// SetProgressData 设置进度数据（领域值对象）
func (r *Requirement) SetProgressData(data *ProgressData) {
	r.progressData = data
	r.updatedAt = time.Now()
}
...
func (r *Requirement) ToSnapshot() RequirementSnapshot {
	return RequirementSnapshot{
		...
		ProgressData:           func() string { if r.progressData != nil { return r.progressData.ToJSON() }; return "" }(),
	}
}
...
func (r *Requirement) FromSnapshot(s RequirementSnapshot) error {
	...
	if s.ProgressData != "" {
		r.progressData = ProgressDataFromJSON(s.ProgressData)
	} else {
		r.progressData = nil
	}
	return nil
